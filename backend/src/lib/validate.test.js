...
describe('isValidEmployeeId', () => {
    test('accepts EMP-### only (the shapes seen in live data are rejected)', () => {
        for (const id of ['EL107', 'EL101', 'EL1111', ' EL999 ', 'EMP-001', 'EMP-047', ' EMP-999 ']) assert.equal(isValidEmployeeId(id), true, id);
        for (const id of ['Emp-023', 'el107', 'EL-107', 'EL10700', 'EL10', 'ENG _556', 'Eng006', 'Eng-aaa', 'SKY_11', 'Amresh', 'EMP-1', 'EMP-0001', '', null]) assert.equal(isValidEmployeeId(id), false, String(id));
    });
});
