    ...
    return (
        <Modal open={!!user} onClose={onCancel}>
            <div className="flex flex-col items-center text-center gap-5">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center border transition-all ${hardDelete ? 'bg-red-500/20 border-red-500/30' : 'bg-amber-500/10 border-amber-500/20'}`}>
                    <AlertTriangle className={`w-8 h-8 ${hardDelete ? 'text-red-500' : 'text-amber-500'}`} />
                </div>
                <div>
                    <h2 className="text-xl font-black text-white mb-2">
                        {hardDelete ? 'Permanent Purge' : 'Deactivate Employee'}
                    </h2>
                    <p className="text-xs text-slate-400 leading-relaxed px-4">
                        {hardDelete 
                            ? `This will completely ERASE ${user?.name} and all their attendance/access history from the database forever.` 
                            : `This will hide ${user?.name} from the active list but keep their historical attendance data safe.`}
                    </p>
                </div>

                <div className="w-full bg-slate-900/50 border border-white/[0.05] rounded-2xl p-4 space-y-3">
                    <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${!hardDelete ? 'bg-blue-500/10 border-blue-500/30' : 'border-transparent'}`}>
                        <input type="radio" name="delType" checked={!hardDelete} onChange={() => setHardDelete(false)} className="accent-blue-500" />
                        <div className="text-left">
                            <div className="text-[11px] font-black text-blue-400 uppercase tracking-widest">Soft Delete (Safe)</div>
                            <div className="text-[10px] text-slate-500">Hide from panel, keep historical data.</div>
                        </div>
                    </label>

                    <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${hardDelete ? 'bg-red-500/10 border-red-500/30' : 'border-transparent'}`}>
                        <input type="radio" name="delType" checked={hardDelete} onChange={() => setHardDelete(true)} className="accent-red-500" />
                        <div className="text-left">
                            <div className="text-[11px] font-black text-red-400 uppercase tracking-widest">Hard Delete (Danger)</div>
                            <div className="text-[10px] text-slate-500">Permanently erase everything from Supabase.</div>
                        </div>
                    </label>
                </div>
                ...
                <div className="flex gap-3 w-full pt-2">
                    ...
                    <button onClick={() => onConfirm(hardDelete)}
                        className={`flex-1 py-3 rounded-xl text-white text-sm font-black transition-all shadow-lg 
                            ${hardDelete ? 'bg-red-600 hover:bg-red-500 shadow-red-600/20' : 'bg-blue-600 hover:bg-blue-500 shadow-blue-600/20'}`}>
                        {hardDelete ? 'Erase Forever' : 'Deactivate Now'}
                    </button>
                </div>
            </div>
        </Modal>
            ...
            <div className="flex items-center">
                {user.face_registered ? (
                    <div className="flex items-center bg-emerald-500/10 border border-emerald-500/20 rounded-full overflow-hidden">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); onEnrollFace(user); }} title="Update Face Biometric"
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onEnrollFace(user); }}
            ...
            <div className="flex items-center">
                {user.fingerprint_registered ? (
                    <div className="flex items-center bg-violet-500/10 border border-violet-500/20 rounded-full overflow-hidden">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); onEnrollFP(user); }} title="Update Fingerprint"
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onEnrollFP(user); }}
...
export default function Users() {
    ...
    const handleDeleteConfirm = async (hard = false) => {
        ...
        try {
            await apiService.deleteUser(target.id, hard);
            setUsers(u => u.filter(x => x.id !== target.id));
            addToast(`${target.name} ${hard ? 'permanently purged' : 'deactivated'}.`);
        } catch (err) {
            addToast(err.response?.data?.message || err.message, 'error');
        } finally { setActionLoading(null); }
    };
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {loading ? (
                                ...
                                return (
                                    <tr key={user.id}
                                        onClick={() => navigate(`/admin/access/employee/${user.employee_id || user.id}`)}
                                        className={`group transition-colors cursor-pointer ${isDisabled ? 'opacity-50' : 'hover:bg-slate-50'}`}>
                                        ...
                                        <td className="px-4 md:px-8 py-4 text-right">
                                            <div className="flex items-center justify-end gap-0.5 md:gap-1">
                                                <button onClick={(e) => { e.stopPropagation(); setEditTarget(user); }} title="Edit"
                                                    className="p-1.5 md:p-2 rounded-lg hover:bg-blue-500/10 text-slate-500 hover:text-emerald-500 transition-all">
                                                    <Edit2 className="w-3.5 h-3.5 md:w-4 md:h-4" />
                                                </button>
                                                <button onClick={(e) => { e.stopPropagation(); handleDisableToggle(user); }} disabled={isActioning}
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(user); }} disabled={isActioning} title="Delete"
