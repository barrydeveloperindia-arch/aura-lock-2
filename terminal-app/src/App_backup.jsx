...
function LiveClock() {
    ...
    return (
        <div className="text-center">
            <div className="text-7xl font-black tabular-nums tracking-tight text-slate-900">
...
export default function App() {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    return (
        <div className="w-screen h-screen kiosk-gradient flex flex-col items-center justify-center p-8 text-slate-900 relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 px-8 py-5 flex items-center justify-between border-b border-white/[0.04] z-10">
                <div onClick={() => { if (view === 'home') setView('admin_auth'); }} className="cursor-pointer hover:opacity-80 transition-opacity">
                    <div className="text-xs font-black uppercase tracking-[0.3em] text-emerald-600">AuraLock</div>
            ...
            <AnimatePresence mode="wait">
                ...
                {view === 'admin_auth' && (
                    <motion.div key="admin_auth" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="glass p-8 rounded-3xl w-full max-w-md flex flex-col gap-6 items-center z-20">
                        <div className="flex items-center justify-between w-full border-b border-slate-200 pb-4">
                            <h2 className="text-lg font-black flex items-center gap-2"><ShieldAlert size={20} className="text-emerald-600" /> Admin Access</h2>
                            <button onClick={reset} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={20} /></button>
                        </div>
                        <input type="password" placeholder="Enter Admin PIN" className="w-full bg-white border-slate-200 rounded-xl p-4 text-center text-xl tracking-widest focus:outline-none focus:border-blue-500/50" value={adminPin} onChange={e => setAdminPin(e.target.value)} autoFocus />
                        <button onClick={() => { if (adminPin === '1234') { setView('admin_select'); setAdminPin(''); } else { setMessage('Invalid PIN'); setTimeout(() => setMessage(''), 2000); } }} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 rounded-xl font-black text-slate-900 uppercase tracking-widest transition-colors flex items-center justify-center gap-2"><Unlock size={18} /> Authenticate</button>
                ...
                {view === 'admin_select' && (
                    <motion.div key="admin_select" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="glass p-8 rounded-3xl w-full max-w-2xl flex flex-col gap-5 z-20">
                        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                            <h2 className="text-lg font-black flex items-center gap-2"><UserPlus size={20} className="text-emerald-600" /> Select Employee for Face Registration</h2>
                            <button onClick={reset} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={20} /></button>
                        </div>
                        <input type="text" placeholder="Search by name…" className="w-full bg-white border-slate-200 rounded-xl p-4 text-base focus:outline-none focus:border-blue-500/50 placeholder:text-slate-600 transition-colors" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} autoFocus />
                        <div className="grid grid-cols-2 gap-3 max-h-[380px] overflow-y-auto pr-1 custom-scrollbar">
                            {employees.filter(e => e.name.toLowerCase().includes(searchTerm.toLowerCase())).map(emp => (
                                <button key={emp.id} onClick={() => { setSelectedEmp(emp); setView('admin_scan'); setMessage('Ready to capture'); }} className="flex items-center gap-3 p-4 glass hover:glass-active rounded-2xl transition-all text-left">
                                    <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden shrink-0"><img src={emp.image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(emp.name)}&background=1e293b&color=94a3b8`} alt="" className="w-full h-full object-cover" /></div>
                ...
                {view === 'admin_scan' && (
                    <motion.div key="admin_scan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center gap-10 z-20">
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Register: {selectedEmp?.name}</h2>
                        <div className="relative w-64 h-64">
                            {/* Corner brackets */}
                            {[['top-0 left-0', 'border-t-2 border-l-2'], ['top-0 right-0', 'border-t-2 border-r-2'], ['bottom-0 left-0', 'border-b-2 border-l-2'], ['bottom-0 right-0', 'border-b-2 border-r-2']].map(([pos, br], i) => (<div key={i} className={`absolute w-8 h-8 ${pos} ${br} border-emerald-400 rounded-sm`} />))}
                            <div className="w-full h-full rounded-2xl flex items-center justify-center bg-emerald-500/5 border border-blue-500/10 overflow-hidden relative">
                        ...
                        <div className="flex flex-col gap-4 text-center w-full max-w-sm">
                            ...
                            <button onClick={captureAndRegister} disabled={loading} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 rounded-xl font-black text-slate-900 uppercase tracking-widest transition-colors flex items-center justify-center">
                                {loading ? 'Processing...' : 'Capture & Save'}
                            </button>
                            <button onClick={reset} disabled={loading} className="px-6 py-2 bg-slate-100 rounded-xl font-bold text-slate-900 uppercase text-[10px] tracking-widest hover:bg-slate-200 transition-colors">Cancel</button>
                        </div>
                    </motion.div>
                )}
                ...
                {view === 'home' && (
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-3 mt-4">
                        {/* BLE Indicator */}
                        <div className="flex items-center gap-2 px-4 py-2 bg-white border-slate-200 rounded-full">
                            {bleStatus === 'connected' ? (
                                ...
                                <><Bluetooth size={14} className="text-emerald-600 animate-pulse" /><span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Door Ready</span></>
                            ) : bleStatus === 'searching' ? (
                                <><Search size={14} className="text-emerald-600/50 animate-spin" /><span className="text-[9px] font-black text-emerald-600/50 uppercase tracking-widest">Searching Lock...</span></>
                            ) : (
                        ...
                        <div className="flex items-center gap-2 px-4 py-2 bg-white border-slate-200 rounded-full">
                            {biometricStatus === 'online' ? (
                                <><Cpu size={14} className="text-emerald-600" /><span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">AI Engine Online</span></>
                        ...
                        <div className="w-full">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <motion.button
                                    ...
                                    <div className="p-5 rounded-2xl bg-emerald-500/10 group-hover:bg-emerald-500/20 transition-colors border border-blue-500/20">
                                        <Camera size={56} className="text-emerald-600" />
                                    </div>
                                    <div className="text-center">
                ...
                {view === 'fingerprint' && (
                    <motion.div key="fingerprint"
                        ...
                        <input type="text" placeholder="Search by name…"
                            className="w-full bg-white border-slate-200 rounded-xl p-4 text-base focus:outline-none focus:border-emerald-500/50 placeholder:text-slate-600 transition-colors"
                    ...
                    <motion.div key="face"
                        ...
                        <div className="relative w-64 h-64">
                            ...
                            ['bottom-0 left-0', 'border-b-2 border-l-2'], ['bottom-0 right-0', 'border-b-2 border-r-2']].map(([pos, br], i) => (
                                <div key={i} className={`absolute w-8 h-8 ${pos} ${br} border-emerald-400 rounded-sm`} />
                            ))}
                            <div className="w-full h-full rounded-2xl flex items-center justify-center bg-emerald-500/5 border border-blue-500/10 overflow-hidden relative">
                                <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover" style={{ transform: 'scaleX(-1)' }} />
                                {/* Face Guide Overlay */}
                        ...
                        <button onClick={reset} className="px-6 py-2 bg-slate-100 rounded-xl font-bold text-slate-900 uppercase text-[10px] tracking-widest hover:bg-slate-200 transition-colors">Cancel</button>
                    </motion.div>
                )}
                ...
                {view === 'error' && (
                    <motion.div key="error"
                        ...
                        <motion.button
                            ...
                            onClick={reset}
                            className="px-8 py-3 bg-slate-50 hover:bg-white/[0.1] border border-white/[0.08] rounded-2xl font-black text-sm uppercase tracking-widest transition-all">
                            Try Again
                        </motion.button>
