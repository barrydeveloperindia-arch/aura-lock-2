...
const MethodBadge = ({ method }) => {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 text-xs font-bold ${m.cls}`}>
...
export default function Logs() {
    ...
    const inputCls = 'bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 transition-colors';
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mb-1 tracking-tight">Security Audit Log</h1>
                    <p className="text-slate-500 text-xs md:text-sm">Real-time biometric access events &mdash; every scan attempt recorded</p>
                </div>
                <button onClick={() => fetchLogs(true)} disabled={refreshing}
                    className="flex items-center gap-2 px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-slate-600 hover:text-slate-900 text-xs font-bold transition-all disabled:opacity-50">
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <StatPill label="Ambiguous" value={ambiguous} color="text-amber-700" icon={ShieldQuestion} />
            </div>
            ...
            <div className="card !p-0 overflow-hidden">
                ...
                <div className="px-6 py-3 border-b border-slate-200 flex items-center justify-between bg-slate-50">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-500">Live Access Stream</span>
                    </div>
                    <span className="text-xs text-slate-600 font-bold">{total} total events</span>
                </div>
                            ...
                            {Array.from({ length: Math.min(7, totalPages) }, (_, i) => {
                                ...
                                return (
                                    <button key={pg} onClick={() => setPage(pg)}
                                        className={`w-7 h-7 rounded-lg text-xs font-bold transition-all ${pg === page
