...
.NOTES
  - Keeps the service's existing env (SUPABASE_URL / SUPABASE_KEY) and only sets
    ENGINE_KEY from backend\.env, so the backend and engine share the secret.
...
if ($Stage) {
    $key = ""
    foreach ($line in Get-Content $EnvFile) { if ($line -match '^ENGINE_KEY=(.+)$') { $key = $Matches[1].Trim() } }
    ...
    try {
        gcloud run deploy $Service --source . --region $Region --project $Project `
            --no-traffic --tag next --update-env-vars "ENGINE_KEY=$key" --quiet
